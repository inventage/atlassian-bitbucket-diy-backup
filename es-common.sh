...
function check_es_index_exists {
    ...
    if [ "$(echo "${es_response}" | jq -r '.error .type' 2>/dev/null)" != "index_not_found_exception" ]; then
...
function check_es_needs_configuration {
    ...
    if [ "$(echo "${es_response}" | jq -r '.error .type' 2>/dev/null)" = "repository_missing_exception" ]; then
...
EOF
    ...
    if [ "$(echo "${es_response}" | jq -r '.accepted' 2>/dev/null)" != "true" ]; then
...
EOF
    ...
    if [ "$(echo "${es_response}" | jq -r '.snapshot.snapshot' 2>/dev/null)" != "${snapshot_name}" ]; then
...
function get_es_snapshots {
    ...
    local snapshots=$(echo "${es_response}" | jq -r '.[] | sort_by(.start_time_in_millis) | reverse | .[]  | .snapshot' | grep "^${SNAPSHOT_TAG_PREFIX}" 2>/dev/null)
    if [  -z "${snapshots}" ]; then
